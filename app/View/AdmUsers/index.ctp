		...
		<?php foreach ($admUsers as $admUser): ?>
			<tr>
				...
				<td class="actions">
					...
					echo ' '.$this->Html->link('<i class="icon-briefcase icon-white"></i>'.__(''),  array_merge($url,$parameters), array('class'=>'btn btn-primary', 'escape'=>false, 'title'=>'Asignar roles')); 
					echo ' '.$this->Form->postLink('<i class="icon-trash icon-white"></i>', array('action' => 'delete', $admUser['AdmUser']['id']), array('class'=>'btn btn-danger', 'escape'=>false, 'title'=>'Eliminar'), __('¿Esta seguro de borrar este usuario?', $admUser['AdmUser']['id']));
					?>
				</td>
			</tr>
		<?php endforeach; ?>
